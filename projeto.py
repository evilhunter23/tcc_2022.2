...
#---------- Configurando plano de fundo da página -------------#
...
#---------- Input pricipal -------------#
sigla = col1.text_input('Digite um ticker de empresa', 'pssa3.sa')
...
#---------- Criando a coluna Futuro e atribuindo as features e label -------------#
...
prec = prec*100
print(f'A precisão do modelo na ação {sigla} é de {prec}%')
print(f'O erro médio é: {media}')

#-----------------------------------------#

#---------- Montando as variáveis para os gráficos -------------#
n = previsao.size
treino = treino_y#.iloc[:tam_treino]
validacao = y.iloc[tam_treino:]
real = X['Close'].iloc[tam_treino:]
validacao['Previsto'] = previsao
#---------------------------------------------------------------#

#---------- Grafico demonstrativo do modelo -------------#
def graf_1():
    fig1 = go.Figure()
